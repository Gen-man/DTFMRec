...
def init_logger(config):
    ...
    """
    reset_logging()
    LOGROOT = './log/{}/'.format(config['dataset'])
    dir_name = os.path.dirname(LOGROOT)
    if not os.path.exists(dir_name):
        os.makedirs(dir_name)

    # logfilename = '{}-{}-{}.log'.format(config['model'], config['dataset'], get_local_time())
    logfilename = '{}-{}-{}.log'.format(config['dataset'], config['name'], get_local_time())
    ...
    logger = logging.getLogger(config['dataset'])
