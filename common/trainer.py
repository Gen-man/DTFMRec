...
class Trainer(AbstractTrainer):  # 初始化训练方法
   ...
   and some other features helpful for model training and evaluation.
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None):  # 训练一个周期
        ...
        for batch_idx, interaction in enumerate(train_data):
            ...
            if self._check_nan(loss):  # 检查是否有NaN值
    ...
    def _valid_epoch(self, valid_data):
        ...
        # valid_result, user_embs = self.evaluate(valid_data)
    ...
    def _generate_train_loss_output(self, epoch_idx, s_time, e_time, losses):  # 生成训练损失输出
    ...
    def fit(self, train_data, valid_data=None, test_data=None, saved=False, verbose=True):
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            train_loss, _ = self._train_epoch(train_data, epoch_idx)  # train_epoch
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                # valid_score, valid_result, user_embs = self._valid_epoch(valid_data)
                ...
                # _, test_result, _ = self._valid_epoch(test_data)
                if verbose:
                ...
                if update_flag:
                    ...
                    # # 保存当前最佳的user_embs
                    # best_user_embs_filename = f'./saved_model/baby/best_user_embs_epoch_{epoch_idx}.pth'
                    # torch.save(user_embs, best_user_embs_filename)  # 保存最佳的user_embs
                    # success_message = f'Successfully saved best user_embs to {best_user_embs_filename}'
                    # if verbose:
                    ...
                    #     model_dir = f"./saved_model/{dataset_name}"
                    #     os.makedirs(model_dir, exist_ok=True)
                    #     best_model_save_path = os.path.join(model_dir, "best_model.pth")
                    #     self.save_model(self.model, best_model_save_path)
    ...
    def evaluate(self, eval_data, is_test=False, idx=0):
        ...
        for batch_idx, batched_data in enumerate(eval_data):
            ...
            # scores, user_embs = self.model.full_sort_predict(batched_data)
        ...
        # return self.evaluator.evaluate(batch_matrix_list, eval_data, is_test=is_test, idx=idx), user_embs
