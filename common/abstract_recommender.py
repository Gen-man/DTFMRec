...
class GeneralRecommender(AbstractRecommender):
    ...
    def __init__(self, config, dataloader):
        ...
        # load dataset info
        self.USER_ID = config['USER_ID_FIELD'] # 获取user_id
        self.ITEM_ID = config['ITEM_ID_FIELD'] # 获取item_id
        self.NEG_ITEM_ID = config['NEG_PREFIX'] + self.ITEM_ID # 获取负样本的item_id
        self.n_users = dataloader.dataset.get_user_num() # 获取user数量
        self.n_items = dataloader.dataset.get_item_num() # 获取item数量

        # load parameters info 加载参数
        self.batch_size = config['train_batch_size']
        self.device = config['device']

        # load encoded features here  加载视觉和文本编码特征功能
